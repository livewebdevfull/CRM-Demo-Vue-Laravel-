...
<template>
  <vx-card title="Formulario de modificación">

    <div class="mt-0">
      <form-wizard color="rgba(var(--vs-primary), 1)" errorColor="rgba(var(--vs-danger), 1)" :title="null" :subtitle="null" finishButtonText="Guardar" nextButtonText="Siguiente" backButtonText="Atras">
        <tab-content title="Información de identificación" class="mb-5" icon="feather icon-user" :before-change="validateStep1" v-if="this.$store.state.AppActiveUser.userRole === 'sales_manager'">
          ...
          <form data-vv-scope="step-1">
            ...
            <div class="vx-row">
              <div class="vx-col md:w-1/3 w-full mt-5">
                <vs-input label="Nombre" v-model="firstName" class="w-full" name="first_name" v-validate="'required|alpha'" />
                <span class="text-danger">{{ errors.first('step-1.first_name') }}</span>
              </div>
              <div class="vx-col md:w-1/3 w-full mt-5">
                <vs-input label="Apellido"  v-model="lastName" class="w-full" name="last_name" v-validate="'required|alpha'" />
              ...
              <div class="vx-col md:w-1/4 w-full mt-5">
                <vs-select v-model="docType" class="w-full select-large" label="Tipo de documento">
                  <vs-select-item :key="index" :value="item.value" :text="item.text" v-for="(item,index) in docTypes" class="w-full" />
                </vs-select>
              </div>
              <div class="vx-col md:w-1/4 w-full mt-5">
                <vs-input label="Numero de documento" v-model="docNumber" class="w-full" name="doc_number" type="number" v-validate="'required'" />
                <span class="text-danger">{{ errors.first('step-1.doc_number') }}</span>
              </div>
              <div class="vx-col md:w-1/6 w-full mt-5">
                <vs-input label="CUIL" v-model="cuilFirst" class="w-full" name="cuil_first" type="number" v-validate="'required'" />
              ...
              <div class="vx-col md:w-1/6 w-full mt-5">
                <vs-input label=" " v-model="cuilSecond" class="w-full" name="cuil_second" type="number" v-validate="'required'" />
        ...
        <!-- tab 2 content -->
        <tab-content title="Datos Personales" class="mb-5" icon="feather icon-info" :before-change="validateStep2">
          <form data-vv-scope="step-2">
            <div class="vx-row">
              <div class="vx-col md:w-1/2 w-full mt-5">
                <vs-select v-model="civilState" class="w-full select-large" label="Civil state">
                  <vs-select-item :key="index" :value="item.value" :text="item.text" v-for="(item,index) in civilStates" class="w-full" />
                </vs-select>
              </div>
              <div class="vx-col md:w-1/2 w-full mt-5">
                <vs-input type="email" label="Correo electronico"  v-model="email" class="w-full" name="email" v-validate="'required|email'" />
            ...
            <div class="vx-row">
              <div class="vx-col md:w-1/3 w-full mt-2">
                <vs-input label="Calle" v-model="addressStreet" class="w-full" name="address_street" v-validate="'required'" />
                <span class="text-danger">{{ errors.first('step-2.address_street') }}</span>
              </div>
              <div class="vx-col md:w-1/3 w-full mt-2">
                <vs-input label="Numero" v-model="addressNumber" class="w-full" type="number" name="address_number" v-validate="'required'" />
                <span class="text-danger">{{ errors.first('step-2.address_number') }}</span>
              </div>
              <div class="vx-col md:w-1/3 w-full mt-2">
                <vs-input label="Barrio" v-model="addressNeighbour" class="w-full" name="address_neighbour" v-validate="'required'" />
                <span class="text-danger">{{ errors.first('step-2.address_neighbour') }}</span>
              </div>
              <div class="vx-col md:w-1/3 w-full mt-2">
                <vs-input label="Localidad" v-model="addressLocation" class="w-full" name="address_location" v-validate="'required'" />
                <span class="text-danger">{{ errors.first('step-2.address_location') }}</span>
              </div>
              <div class="vx-col md:w-1/3 w-full mt-2">
                <vs-input label="CP" v-model="addressCp" class="w-full" name="address_cp" v-validate="'required'" />
                <span class="text-danger">{{ errors.first('step-2.address_cp') }}</span>
              </div>
              <div class="vx-col md:w-1/3 w-full mt-2">
                <vs-input label="Provincia" v-model="addressProvince" class="w-full" name="address_province" v-validate="'required'" />
            ...
            <div class="vx-row">
              <div class="vx-col md:w-1/3 w-full mt-2">
                <vs-input label="Telefono1" v-model="phoneNumber1" class="w-full" name="phone_number" v-validate="'required'" />
                <span class="text-danger">{{ errors.first('step-2.phone_number') }}</span>
              </div>
              <div class="vx-col md:w-1/3 w-full mt-2">
                <vs-input label="Telefono2" v-model="phoneNumber2" class="w-full" type="number" name="phone_number2" />
              </div>
              <div class="vx-col md:w-1/3 w-full mt-2">
                <vs-input label="Telefono3" v-model="phoneNumber3" class="w-full" name="phone_number3" />
              </div>
            </div>
          </form>
        </tab-content>
        ...
        <!-- tab 3 content -->
        <tab-content title="Afiliación" class="mb-5" icon="feather icon-gitlab" :before-change="validateStep3">
          <form data-vv-scope="step-3">
            <div class="vx-row">
              <div class="vx-col md:w-1/2 w-full mt-5">
                <vs-select v-model="membershipClass" class="w-full select-large" label="Clase">
                  <vs-select-item :key="index" :value="item.value" :text="item.text" v-for="(item,index) in membershipClasses" class="w-full" />
                </vs-select>
              </div>
              <div class="vx-col md:w-1/2 w-full mt-5">
                <vs-select v-model="membershipBranch" class="w-full select-large" label="Sucursal">
                  <vs-select-item :key="index" :value="item.value" :text="item.text" v-for="(item,index) in membershipBranches" class="w-full" />
                </vs-select>
              </div>
              <div class="vx-col md:w-1/1 w-full mt-2">
                <vs-input label="Cuota" v-model="membershipFee" class="w-full" type="number" name="membership_fee" v-validate="'required'" />
        ...
        <!-- tab 4 content -->
        <tab-content title="Datos bancarios" class="mb-5" icon="feather icon-dollar-sign" :before-change="validateStep4" v-if="this.$store.state.AppActiveUser.userRole === 'sales_manager'">
          <form data-vv-scope="step-4">
            ...
            <div class="vx-row border border-primary" style="border: 1px solid;border-radius: 16px;padding: 20px; margin-bottom: 20px;" :key="index" v-for="(item,index) in bankInfo">
              <div class="vx-col md:w-1/4 w-full mt-5">
                <vs-input label="Banco" v-model="item.bankName" class="w-full" name="bank_name" v-validate="'required'" />
              </div>
              <div class="vx-col md:w-1/4 w-full mt-5">
                <vs-input label="CBU" v-model="item.bankNumber" class="w-full" type="number" name="bank_number" v-validate="'required'" />
              </div>
              <div class="vx-col md:w-1/6 w-full mt-5">
                <vs-input label="Numero de Cuenta" v-model="item.bankAccount" class="w-full" type="number" name="bank_account" />
              </div>
              <div class="vx-col md:w-1/6 w-full mt-5">
                <vs-input label="Numero de Cliente" v-model="item.bankClient" class="w-full" type="number" name="bank_client" />
              </div>
...
export default {
    ...
    async created() {
      ...
      await axios.get("/api/client-list/client/"+this.$route.query.id)
        .then((response) => {
          if(response.data) {
            ...
            bankInfo.forEach((item)=> {
              var info = {bankName:item.bank_name, bankNumber:item.bank_number, bankAccount:item.bank_account, bankClient:item.bank_client};
        ...
        validateStep3() {
            return new Promise((resolve, reject) => {
                this.$validator.validateAll("step-3").then(result => {
                    if (result) {
                        if(this.$store.state.AppActiveUser.userRole === 'vendor') {
                          var data = new FormData(); 
                          if(this.file)
                            data.append('file', this.file)
                          else
                            data.append('avatar', this.userImg)
                          data.apeend('user_id', this.userId)
                          ...
                          axios.post("/api/client-list/client/update/"+this.$route.query.id, data, {headers: {'Content-Type': 'multipart/form-data'}})
                          .then((response) => {
                            if(response.data) {
